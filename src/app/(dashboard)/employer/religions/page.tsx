...
export default function ReligionsPage() {
  ...
  const [formData, setFormData] = useState<Omit<Religion, 'id'>>({
    name: ''
  });

  const handleOpenModal = (religion?: Religion) => {
    if (religion) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this religion?')) {
      try {
        await religionService.delete(id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentReligion) {
        const updated = await religionService.update(currentReligion.id, formData);
        setReligions(prev => prev.map(r => r.id === updated.id ? updated : r));
      } else {
        const newReligion = await religionService.create(formData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Religions</h1>
          <p className="text-sm text-gray-500 mt-1">Manage employee religion options</p>
        </div>
        <button
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {religions.map((religion) => (
              <tr key={religion.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{religion.name}</td>
              ...
              <form onSubmit={handleSubmit}>
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                    {currentReligion ? 'Edit Religion' : 'Add Religion'}
                  </h3>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  ...
                  >
                    {currentReligion ? 'Save Changes' : 'Add Religion'}
                  </button>
                  <button
                    type="button"
