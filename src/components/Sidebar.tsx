...
export const menuItems = [
  { key: 'dashboard', name: 'Dashboard', href: '/dashboard', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
  {
    key: 'self_service_ess',
  ...
  { 
    ...
    subItems: [
      ...
      { name: 'Timesheet Dashboard', href: '/attendance/timesheets' },
  ...
  { key: 'reports', name: 'Reports', href: '/reports', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  { key: 'settings', name: 'Settings', href: '/settings', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
];
  ...
  useEffect(() => {
    async function fetchMenuConfigs() {
      try {
        ...
        configs.forEach(c => {
          labelMap[c.menu_key] = c.display_name;
        });
        setMenuLabels(labelMap);
      } catch (error) {
    ...
    // Subscribe to menu configuration changes
    const channel = supabase
      .channel('menu-config-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'menu_item_configurations'
        },
        () => {
          fetchMenuConfigs();
        }
    ...
    .filter(item => {
      ...
      if (!hasMenuConfig) {
        ...
        if (userRole === 'Employee') {
          return ['dashboard', 'self_service_ess'].includes(item.key || '');
        }
        if (userRole === 'Manager') {
          return ['dashboard', 'team', 'leave'].includes(item.key || '');
        }
        if (['HR Admin', 'HR Manager'].includes(userRole)) {
          return item.key !== 'settings' && item.key !== 'self_service_ess';
        }
        return item.key !== 'self_service_ess';
      }

      const menuKey = item.key;
      return roleMenuPermissions!.includes(`menu:${menuKey}`);
    })
    .map(item => {
